      ...
      <div style={{ width: "100%", maxWidth: 360 }}>
        {step === "phone" ? (
          <>
            ...
            <div style={{ marginTop: 24, background: "rgba(212,168,67,0.06)", border: "1px solid rgba(212,168,67,0.15)", borderRadius: 12, padding: "14px 16px" }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: "var(--gold)", marginBottom: 8 }}>🔧 測試帳號（Demo）</div>
              {CONTRACTOR_DB.map(c => (
                <div key={c.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--muted)", marginBottom: 4 }}>
                  <span style={{ cursor: "pointer", color: "var(--text)" }} onClick={() => setPhone(c.phone)}>{c.name} — {c.phone}</span>
                  <span>PIN: {c.pin}</span>
                </div>
              ))}
            </div>
